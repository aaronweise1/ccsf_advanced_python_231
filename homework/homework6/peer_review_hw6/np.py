...
# war, the card game of chance where 26 battles take place between rival armies. the
# higher card wins each battle. ties accumulate a bonus to be won at the next battle.
# for each battle, outputs the number of cards left, the two cards drawn, and the win
# totals. if a battle is a tie, its value is accrued towards the next one that is won.
...
while deck:
    ...
    # compare a pair of cards' values, tally scores and adjust bonus there are three
    # possible cases; in case of a win the bonus is paid out, otherwise it rises
    ...
    print('{:55.55}  {} to {}, {} left.'.format(event, scoreA, scoreB,
                                                int(len(deck) / 2)))
