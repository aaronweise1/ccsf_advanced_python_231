...
deck = [ (name + ' of ' + suit, names.index(name) ) for name in names for suit in suits ] #BUG - fixed card value
random.shuffle(deck) #BUG added shuffle
...
while deck:
 ...
 if cardA[1] == cardB[1]:
  ...
  scoreB += 1 + bonus #BUG changed to scoreB
  bonus = 0
  outcome = 'is beaten by' #BUG fixed indent

 # display the outcome of each battle, current winnings, and how much left to be won
 event = "The {cardA} {outcome} the {cardB}!".format( #NOTE made more readable formatting
     cardA=cardA[0], 
     outcome=outcome, 
     cardB=cardB[0],
     )
 print ('{event:55.55}  ${scoreA} to ${scoreB}, ${points_left} left.'.format( #NOTE made more readable formatting
     event=event, 
     scoreA=scoreA, 
     scoreB=scoreB, 
     points_left=int(len(deck)/2), 
     ) 
     )
